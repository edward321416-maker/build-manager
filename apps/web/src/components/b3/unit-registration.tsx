...
export type UnitSubmitResult=
 ...
 |{kind:'invalid'|'denied'|'unauthenticated'|'conflict'|'uncertain'};
...
export function UnitRegistration({orgId,propertyId}:{orgId:string;propertyId:string}){
 ...
 useEffect(()=>{
  ...
  void loadUnitRegistrationAccess(fetch,orgId,propertyId,controller.signal).then(result=>{if(!controller.signal.aborted)setAccess(result.canCreate?{phase:'ready',csrf:result.csrf}:{phase:'denied',csrf:result.csrf});}).catch(error=>{if(!controller.signal.aborted)setAccess({phase:error instanceof Error&&['401','403','404'].includes(error.message)?'denied':'unavailable'});});
 ...
 async function submit(event:React.FormEvent<HTMLFormElement>){
  ...
  if(result.kind==='denied'){
   setAccess({phase:'denied'});setLabel('');setMessage('');
  ...
  else if(result.kind==='denied'||result.kind==='unauthenticated'){setAccess({phase:'denied',csrf:deniedCsrf});setLabel('');setMessage('');}
