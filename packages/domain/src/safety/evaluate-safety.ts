...
/** `null`/`"unknown"` mean the tenant has not established an answer either way. */
...
/**
 * Bounded phrase detection over untrusted tenant text, covering the trigger
 * concepts listed in `product/ai_safety_boundary.md`. Text is data, never
 * instructions. Matching is intentionally conservative and may produce false
 * positives; it is not language understanding or safety certification.
 */
function flagsFromRawText(rawText: string | undefined): SafetyFlag[] {
  ...
  if (GAS_SMELL_PATTERN.test(rawText)) {
    flags.push("GAS_SMELL");
  }
  if (SMOKE_OR_FIRE_PATTERN.test(rawText)) {
    flags.push("SMOKE_OR_FIRE");
  }
  if (ELECTRICAL_WATER_PATTERN.test(rawText)) {
    flags.push("ELECTRICAL_WATER_RISK");
  }
  if (OTHER_URGENT_PATTERN.test(rawText)) {
    flags.push("OTHER_URGENT_HAZARD");
  }
  return flags;
}

/**
 * Deterministic safety gate. No model and no network.
 *
 * `escalated: false` means only that no positive trigger was detected; missing
 * data never establishes low risk, so completeness is a separate gate.
 */
export function evaluateSafety(input: SafetyInput): SafetyResult {
  const flags = new Set<SafetyFlag>();

  if (input.gasSmell === true) {
    flags.add("GAS_SMELL");
  }
  if (input.smokeOrFire === true) {
    flags.add("SMOKE_OR_FIRE");
  }
  if (input.electricalWaterRisk === true) {
    flags.add("ELECTRICAL_WATER_RISK");
  }
  if (input.otherUrgentHazard === true) {
    flags.add("OTHER_URGENT_HAZARD");
  }
  for (const flag of flagsFromRawText(input.rawText)) {
    flags.add(flag);
  }
