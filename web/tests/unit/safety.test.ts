...
describe("evaluateSafety", () => {
  ...
  it("returns no escalation for an ordinary negative answer", () => {
  ...
  it("hard-stops on another urgent policy hazard", () => {
  ...
  it.each([
    ...
    ["There is large-scale water leakage", "OTHER_URGENT_HAZARD"],
  ...
  ] as const)("detects the bounded raw-text hazard %s", (rawText, flag) => {
  ...
  it("does not let an instruction inside tenant text disable a hazard", () => {
    expect(
      evaluateSafety({
  ...
  it("keeps multiple hazards once in deterministic precedence order", () => {
    expect(
      evaluateSafety({
        gasSmell: false,
        smokeOrFire: true,
  ...
  it("reports no positive trigger when safety answers are missing", () => {
  ...
  it("does not escalate unrelated repair text", () => {
